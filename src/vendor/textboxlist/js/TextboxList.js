...
$.TextboxList = function(element, _options){
    ...
    plugins: {},
		...
		encode: function(o){ 
			return $.grep($.map(o, function(v){		
				v = (chk(v[0]) ? v[0] : v[1]);
				return chk(v) ? v.toString().replace(/,/, '').trim() : null;
	...
	var self = this;
	var init = function(){
		//This hides the original input field
		...
		list = $('<ul class="'+ options.prefix +'-bits form-control" />').appendTo(container);
